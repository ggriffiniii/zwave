...
function getSunrise(dateMoment, eventSpec) {
  var date = new Date(dateMoment.year(), dateMoment.month(), dateMoment.date());
  return moment(suncalc.getTimes(date,
                                 eventSpec.latitude,
                                 eventSpec.longitude).sunrise);
}

function getSunset(dateMoment, eventSpec) {
  var date = new Date(dateMoment.year(), dateMoment.month(), dateMoment.date());
  return moment(suncalc.getTimes(date,
                                 eventSpec.latitude,
                                 eventSpec.longitude).sunset);
}
...
function getNextEventTime(eventSpec) {
  ...
  if (eventSpec.type === 'static') {
    ...
    console.log(eventSpec.type + ' event on ' + eventSpec.weekday);
    var nextDate = moment()
        .tz('HST')  // Hawaii has the last sunrise
        .isoWeekday(eventSpec.weekday)
        .endOf('day');
    if (eventSpec.type === 'sunrise') {
      for(var sunrise = getSunrise(nextDate, eventSpec);
          sunrise.isBefore(now);
          nextDate.add('weeks', 1)) {
        sunrise = getSunrise(nextDate, eventSpec);
      }
      return sunrise;
    } else if (eventSpec.type === 'sunset') {
      for(var sunset = getSunset(nextDate, eventSpec);
          sunset.isBefore(now);
          nextDate.add('weeks', 1)) {
        sunset = getSunset(nextDate, eventSpec);
      }
      return sunset;
    }
  }
...
function eventId(eventSpec) {
  ...
  if (eventSpec.type === 'sunset' || eventSpec.type === 'sunrise') {
    ...
    var id = base_id + eventSpec.isoWeekday*60*24;
