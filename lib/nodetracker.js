...
function eventId(eventSpec) {
  ...
  if (eventSpec.type === 'sunset') {
    var sunset_base_id = 200000;
    if ('addMinutes' in eventSpec) {
      return sunset_base_id + eventSpec.addMinutes;
    } else {
      return sunset_base_id;
    }
  } else if (eventSpec.type === 'sunrise') {
    var sunrise_base_id = 300000;
    if ('addMinutes' in eventSpec) {
      return sunrise_base_id + eventSpec.addMinutes;
    } else {
      return sunrise_base_id;
    }
...
baseTracker.prototype.addRecurringEvent = function(nodeId, eventSpec) {
  ...
  self.eventsDB.remove({nodeId: nodeId, eventId: eventId(eventSpec)}, function(err, numRemoved) {
    ...
    if (eventSpec.type === 'sunrise' || eventSpec.type === 'sunset') {
      ...
      dbSpec.weekday = eventSpec.weekday;
